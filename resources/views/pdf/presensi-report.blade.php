...
<body>
    <div class="container">
        ...
        <!-- Presensi Table -->
        <div class="report-title">RIWAYAT PRESENSI</div>
            ...
            <tbody>
                @forelse ($presensi as $index => $item)
                    @php
                        ...
                        $isAbsen = in_array($status, ['izin', 'tidak-hadir']);
                    @endphp
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td>{{ \Carbon\Carbon::parse($item->tanggal)->locale('id')->isoFormat('dddd, DD/MM/YYYY') }}</td>
                        <td class="{{ $status === 'hadir' ? 'status-hadir' : ($status === 'izin' ? 'status-izin' : 'status-tidak-hadir') }}">
                            {{ ucfirst($status) }}
                        </td>
                        <td>{{ $isAbsen ? '-' : $item->jam_masuk }}</td>
                        <td>{{ $isAbsen ? '-' : $item->jam_keluar }}</td>
                        <td>
                            @if($status === 'izin')

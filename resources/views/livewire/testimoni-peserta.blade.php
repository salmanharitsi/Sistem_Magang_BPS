...
<div class="pegawai-container bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
    <div class="p-4 border-b border-gray-300">
        <h1 class="font-semibold text-lg text-gray-800">Testimoni Peserta</h1>
    </div>
    ...
    <div class="overflow-x-auto min-h-screen-half">
        <table class="w-full text-sm text-left rtl:text-left">
            ...
            <tbody>
                @forelse ($nonDisplayedTestimonis as $item)
                    <tr class="bg-white border-b hover:bg-gray-50">
                        ...
                        <td class="py-4 px-6">
                            <div class="flex gap-2 w-fit mx-auto items-center">
                                <button wire:click="confirmDisplay('{{ $item->id }}')"
                ...
                @empty
                    <tr>
                        <td colspan="4" class="text-center py-10 text-gray-400">
                            ...
                            <p class="font-semibold">Data testimoni tidak ditemukan</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        {{ $nonDisplayedTestimonis->links('vendor.pagination.custom-pagination') }}
    </div>

    <!-- Modal Konfirmasi Tampilkan -->
    @if($showConfirmModal)
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[1000]">
            <div class="bg-white rounded-lg shadow-lg w-11/12 md:w-1/3 p-5">
                <div class="flex justify-between items-center border-b pb-4 mb-5">
                    <h2 class="text-lg font-semibold">Konfirmasi Tampilkan Testimoni</h2>
                    <button wire:click="$set('showConfirmModal', false)" class="text-gray-500 hover:text-gray-700">
                        <i class="ti ti-x"></i>
                    </button>
                </div>
                
                <div class="mb-6">
                    <p>Apakah Anda yakin ingin menampilkan testimoni ini di landing page?</p>
                </div>
                
                <div class="flex justify-end space-x-3">
                    <button wire:click="$set('showConfirmModal', false)" type="button" class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 border border-gray-300 rounded-lg hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                        Batal
                    </button>
                    <button wire:click="toggleDisplay" type="button" class="px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                        Ya, Tampilkan
                    </button>
                </div>
            </div>

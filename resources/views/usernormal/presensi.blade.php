        ...
        @if (Carbon::parse($magang->tanggal_mulai)->isFuture())
            <div class="col-span-4 card rounded-lg bg-white p-5 h-full dark:bg-[#14181b] transition-all duration-200">
                <div class="w-full h-fit flex gap-3 items-start lg:items-center p-3 bg-blue-100 rounded-lg border text-blue-700 border-blue-700">
                    ...
                    <p class="text-sm">Presensi akan tersedia pada <span class="font-bold">{{ Carbon::parse($magang->tanggal_mulai)->translatedFormat('j F Y') }}</span></p>
                </div>
            </div>
        @else
            <div class="col-span-4 card rounded-lg bg-white p-5 h-full dark:bg-[#14181b] transition-all duration-200">
                <div class="w-full h-fit flex gap-3 items-start lg:items-center p-3 bg-amber-100 rounded-lg border text-amber-700 border-amber-700">
                    ...
                    <p class="text-sm">Pastikan kamu berada dalam radius kantor untuk melakukan presensi dan lapor pulang</p>
                </div>
            </div>
            <!-- Start coding here -->
            <div class="col-span-4">
                @livewire('show-all-presensi')
            </div>
        @endif
    </div>
@endsection
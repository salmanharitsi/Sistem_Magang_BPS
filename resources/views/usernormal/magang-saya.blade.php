...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="col-span-3 card rounded-lg bg-white p-5 h-full dark:bg-[#14181b] transition-all duration-200">
            <a href="/magang" class="pjax-link w-fit flex items-center gap-1">
        ...
        <div class="col-span-3 md:col-span-1 card rounded-lg bg-white p-5 ">
    ...
    @if (Carbon::parse($magang->tanggal_mulai)->isFuture())
        <div class="mt-6 card rounded-lg bg-white p-5 h-full dark:bg-[#14181b] transition-all duration-200">
            <div class="w-full h-fit flex gap-3 items-start lg:items-center p-3 bg-blue-100 rounded-lg border text-blue-700 border-blue-700">
                ...
                <p class="text-sm">Kamu akan memulai magang pada <span class="font-bold">{{ Carbon::parse($magang->tanggal_mulai)->translatedFormat('j F Y') }}</span></p>
            </div>
        </div>
    @endif
    ...
    @elseif (!$magang->laporan_magang && Carbon::parse($magang->tanggal_selesai)->addDays()->isPast())
        <div class="mt-6 col-span-4 card rounded-lg bg-white p-5 h-full dark:bg-[#14181b] transition-all duration-200">
            <div class="w-full h-fit p-3 flex flex-col md:flex-row items-start gap-3 md:items-center justify-between bg-amber-100 rounded-lg border text-amber-700 border-amber-700">
                <div class="flex gap-3 items-start lg:items-center">
                    ...
                    <p class="text-sm">Kamu belum submit laporan akhir magang</p>
                </div>
                <a href="/dashboard"
                    class="pjax-link bg-amber-700 ml-7 md:ml-0 border border-transparent px-3 py-1 rounded-lg text-white hover:bg-amber-100 hover:border hover:border-amber-700 hover:text-amber-700 transition-all duration-200">
                    <p class="text-sm whitespace-nowrap">Submit Dokumen</p>
                </a>
            </div>
        </div>
    @endif
